...
def main():
    ...
    for drink in chain.from_iterable(all_drinks_list):
        if drink["strInstructionsDE"]:
            ...
            # Insert ingredients
            drink_counter = 1
            # While loop with counter to insert ingredients because a drink hardly ever has the max ingredients (15)
            while drink[f"strIngredient{drink_counter}"]:
                ...
                measurement_arguments = [
                    drink["idDrink"], drink_ingredients_dict[ingredient_name],
                    converted_measurement, original_measurement
                ]
    ...
    log.info("Number of drinks inserted:")
    log.info(to_dataframe(drinks))

    ingredients = handle_query(cursor, count_ingredients_query)
    log.info("Number of ingredients inserted:")
    log.info(to_dataframe(ingredients))

    measurements = handle_query(cursor, count_measurements_query)
    log.info("Number of measurements inserted:")
    log.info(to_dataframe(measurements))
    ...
    log.info("Which alcoholic drinks can be mixed with lemon and whiskey?")
    log.info(to_dataframe(lemon_and_whiskey))

    # Which drink(s) can be mixed with just 15g of Sambuca?
    sambuca_cocktails = handle_query(cursor, sambuca_cocktails_query)
    log.info("Which drink(s) can be mixed with just 15g of Sambuca?")
    log.info(to_dataframe(sambuca_cocktails))

    # 3. Which drink has the most ingredients?
    most_ingredients = handle_query(cursor, most_ingredients_drink_query)
    log.info("Which drink has the most ingredients?")
    log.info(to_dataframe(most_ingredients))
